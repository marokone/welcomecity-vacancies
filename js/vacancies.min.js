...
class VacancySearchApp {
    ...
    initSupabase() {
        const url = 'https://vhbiezamhpyejdqvvwuj.supabase.co';
        ...
        console.warn('Supabase не найден, использую fetch');
        return null;
    }

    async loadData() {
        try {
            // Попробуем загрузить из Supabase
            const vacancies = await this.loadFromSupabase();
            this.state.allVacancies = vacancies;
            ...
            this.showEmptyState('Не удалось загрузить вакансии');
        }
    }

    async loadFromSupabase() {
        if (!this.state.supabase) {
            const response = await fetch('https://vhbiezamhpyejdqvvwuj.supabase.co/rest/v1/vacancies?select=*&status=eq.active', {
                headers: {
    ...
    startPlaceholderAnimation(input) {
        ...
        update(); // Первое обновление сразу
        
        if (this.state.placeholderInterval) {
            clearInterval(this.state.placeholderInterval);
        }
        
        this.state.placeholderInterval = setInterval(update, 1500); // 1.5 секунды
    }
    ...
    openMobileFilters() {
        ...
        if (modal) {
            ...
            // Назначаем обработчики для мобильных чекбоксов
    ...
    resetFilters() {
        ...
        // Очищаем интервал placeholder при сбросе
        if (this.state.placeholderInterval) {
            clearInterval(this.state.placeholderInterval);
        ...
        // Перезапускаем placeholder анимацию
