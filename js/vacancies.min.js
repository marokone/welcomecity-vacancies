...
// Для страницы https://welcomecity.ru/search-vacancy
...
class VacancySearchApp {
    ...
    async init() {
        console.log('🔍 Инициализация поиска вакансий');
        
        // Инициализация Supabase
        this.state.supabase = this.initSupabase();
        
        if (!this.state.supabase) {
            console.error('❌ Supabase не загружен');
            this.showError('Ошибка загрузки системы');
            return;
        }
        ...
        console.log('✅ Поиск вакансий готов');
    }

    initSupabase() {
        const supabaseUrl = 'https://vhbiezamhpyejdqvvwuj.supabase.co';
        const supabaseKey = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InZoYmllemFtaHB5ZWpkcXZ2d3VqIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NjE2Njc0MDgsImV4cCI6MjA3NzI0MzQwOH0.13h_XJ7kQFtuCjavkOXN9TzXNF2X4jX5-rcNCFiFqO0';
        
        // Если Supabase уже глобально доступен
        if (window.supabase && window.supabase.createClient) {
            console.log('✅ Используем глобальный Supabase');
            return window.supabase;
        }
        
        // Если библиотека Supabase загружена, создаем клиент
        if (window.supabase && typeof window.supabase === 'function') {
            console.log('✅ Создаем клиент Supabase');
            return window.supabase.createClient(supabaseUrl, supabaseKey);
        }
        
        // Fallback: создаем минимальный клиент на fetch
        console.log('⚠️ Supabase JS не найден, использую fetch API');
        return this.createFallbackClient(supabaseUrl, supabaseKey);
    }

    createFallbackClient(url, key) {
        return {
            from: (table) => ({
                select: (columns = '*') => ({
                    eq: (column, value) => ({
                        order: (orderColumn, options = {}) => ({
                            then: async (callback) => {
                                try {
                                    const query = columns === '*' ? '*' : columns.split(',').join(',');
                                    const urlWithParams = `${url}/rest/v1/${table}?select=${query}&${column}=eq.${value}`;
    ...
    async loadData() {
        try {
            console.log('📡 Загрузка вакансий из Supabase...');
            ...
            if (data && data.length > 0) {
                this.state.allVacancies = data.map(vacancy => ({
                    id: vacancy.id,
                    title: vacancy.title || 'Без названия',
                    project: vacancy.project || vacancy.project_name || 'Без проекта',
                    department: vacancy.department || 'Без отдела',
                    description: vacancy.description || ''
                }));
                
                console.log(`✅ Загружено ${this.state.allVacancies.length} вакансий`);
            } else {
                console.log('ℹ️ Нет активных вакансий');
                this.state.allVacancies = [];
                this.showEmptyState('Нет активных вакансий');
            }
    ...
    setupUI() {
        ...
        if (searchInput) {
            ...
            // Запускаем placeholder анимацию
    ...
    renderDropdownOptions(type) {
        ...
        this.state.allVacancies.forEach(v => {
            const key = isProject ? (v.project || 'Без проекта') : v.department;
    ...
    renderResults() {
        ...
        const filtered = this.state.allVacancies.filter(vac => {
            const matchProject = this.state.selectedProjects.length === 0 || 
                this.state.selectedProjects.includes(vac.project || 'Без проекта');
                        ...
                        <div class="vacancy-card-wrapper">
                            <a href="/vacancy?id=${vac.id}" class="vacancy-card">
                                <div class="vacancy-content">
                                    <h3>${vac.title}</h3>
                                    <p class="vacancy-meta">${vac.project || 'Без проекта'} — ${vac.department}</p>
                                </div>
    ...
    updateMobileFilters() {
        ...
        // Обновляем чекбоксы в мобильных dropdown
    ...
    renderMobileOptions() {
        ...
        if (projectOptions) {
            const values = {};
            this.state.allVacancies.forEach(v => {
                const project = v.project || 'Без проекта';
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (!container) {
        console.log('ℹ️ Не страница поиска вакансий');
        return;
    }
    
    console.log('✅ Страница поиска обнаружена, запускаем приложение...');
    
    // Создаем и сохраняем приложение
    window.vacancyListApp = new VacancySearchApp();
    console.log('🚀 Приложение создано:', window.vacancyListApp);
